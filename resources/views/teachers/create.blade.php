...
@section('content')
    ...
    <div class="card">
        <div class="card-header">
            {{ isset($teacher) && !empty($teacher) ? 'Edit' : 'Create' }} Teacher

        </div>
        <div class="card-body">
            {{ $errors }}

            <form role="form" method="post" action="{{ route('teachers.save') }}" enctype="multipart/form-data" data-parsley-validate>
                @csrf
                <input type="hidden" name="id" value="{{ isset($teacher) && !empty($teacher) ? $teacher->id : 0 }}">
                <div class="form-row">
                    <div class="form-group col-md-6">
                        ...
                        @if ($errors->has('name'))
                            <div class="invalid-feedback fadeIn" style="display: block;">{{ $teacher->first('name') }}</div>
                        @endif
                    </div>
                    <div class="form-group col-md-6">
                ...
                <div class="form-row">
                    ...
                    <div class="form-group col-md-6">
                        ...
                        <select class="form-control" id="gender" name="gender" data-parsley-required="true"
                            ...
                            <option value="male" {{ old('gender') == 'male' ? 'selected' : isset($teacher) && $teacher['gender'] == 'male' ? 'selected' : '' }}>Male</option>
                            <option value="female" {{ old('gender') == 'female' ? 'selected' : isset($teacher) && $teacher['gender'] == 'female' ? 'selected' : '' }}>Female</option>
                        </select>
